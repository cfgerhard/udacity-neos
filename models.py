...
class NearEarthObject:
    ...
    def fullname(self):
        ...
        # Used self.designation and self.name to build a fullname for this object.
        fullname = f'{self.designation} {self.name if self.name is not None else ""}'
        return fullname
...
class CloseApproach(NearEarthObject):
    ...
    def __init__(self, des, **info):
        ...
        :param info: A dictionary of excess keyword arguments supplied to the constructor.
        """
        # Assigned information from the arguments passed to the constructor
        ...
        self.time = cd_to_datetime(info.get('time', None))  # Used the cd_to_datetime function for this attribute.
        self.distance = float(info.get('distance', float('nan')))
        self.velocity = float(info.get('velocity', float('nan')))

        # Create an attribute for the referenced NEO, originally None.
        self.neo = ''  # NearEarthObject(self._designation)

        self._fullname = self._designation + (' (' + self.name + ') ' if self.name != '' else '')
